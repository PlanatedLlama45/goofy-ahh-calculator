...
Sprite::Sprite(uint normalTex, bool hoverReact, uint hoverTex, bool holdReact, uint holdTex, glm::vec2 hitboxSize, glm::vec2 pos, float rot, Window &window, uint shader, int reactButton) :
        normalTex(normalTex), hoverReact(hoverReact), hoverTex(hoverTex), holdReact(holdReact), holdTex(holdTex), hitboxSize(hitboxSize), window(window), shader(shader), reactButton(reactButton) {
...
Button::Button(uint normalTex, bool hoverReact, uint hoverTex, bool holdReact, uint holdTex, std::string label, Font& font, glm::vec3 textCol, glm::vec2 textOffset,
        glm::vec2 hitboxSize, glm::vec2 pos, Window &window, uint shader, uint textShader, int reactButton) {
    sprite = new Sprite(normalTex, hoverReact, hoverTex, holdReact, holdTex, hitboxSize, pos, 0.0f, window, shader, reactButton);
